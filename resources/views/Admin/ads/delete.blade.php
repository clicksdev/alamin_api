...
@section("title", "Ads - Delete")
@section("loading_txt", "Delete")

@section("content")
<div class="d-sm-flex align-items-center justify-content-between mb-4">
    <h1 class="h3 mb-0 text-gray-800">Delete Ad</h1>
    <a href="{{ route("admin.ads.show") }}" class="d-none d-sm-inline-block btn btn-sm btn-primary shadow-sm"><i
            class="fas fa-arrow-left fa-sm text-white-50"></i> Back</a>
</div>

<div class="card p-3 mb-3" id="ads_wrapper">
    <div class="card-header mb-3">
        <h3 class="text-danger text-center mb-0">Are you sure you want to delete this ad?</h3>
    </div>
    <div class="d-flex justify-content-between" style="gap: 16px">
        <div class="w-100">
            <div class="form-group w-100">
                <label for="Title" class="form-label">Title</label>
                <input type="text" class="form-control" id="Name"  placeholder="Ad Title" v-model="title">
            </div>
        </div>
        ...
        <input type="file" class="form-control d-none" id="thumbnail"  placeholder="Ad Thumbnail Picture" @change="handleChangeThumbnail">
        </div>
    </div>
    <div class="form-group w-100 d-flex justify-content-center" style="gap: 16px">
        <a href="{{ route("admin.ads.show") }}" class="btn btn-secondary w-25">Cancel</a>
...
createApp({
    ...
    methods: {
        ...
        async deleteCat() {
            ...
            try {
                const response = await axios.post(`{{ route("admin.ads.delete") }}`, {
                    id: this.id
                },
                );
                if (response.data.status === true) {
                    ...
                    setTimeout(() => {
                        $('.loader').fadeOut()
                        $('#errors').fadeOut('slow')
                        window.location.href = '{{ route("admin.ads.show") }}'
                    }, 1300);
                } else {
                    $('.loader').fadeOut()
                    document.getElementById('errors').innerHTML = ''
