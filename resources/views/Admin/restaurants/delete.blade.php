...
@section("title", "restaurants - Delete")
@section("loading_txt", "Delete")

@section("content")
<div class="d-sm-flex align-items-center justify-content-between mb-4">
    <h1 class="h3 mb-0 text-gray-800">Delete restaurant</h1>
...
<div class="card p-3 mb-3" id="restaurants_wrapper">
    <div class="card-header mb-3">
        <h3 class="text-danger text-center mb-0">Are you sure you want to delete this restaurant?</h3>
    </div>
    ...
    <div class="form-group w-100 d-flex justify-content-center" style="gap: 16px">
        <a href="{{ route("admin.restaurants.show") }}" class="btn btn-secondary w-25">Cancel</a>
...
createApp({
    ...
    methods: {
        handleChangeThumbnail(restaurant) {
            this.thumbnail = restaurant.target.files[0]
            this.thumbnail_path = URL.createObjectURL(restaurant.target.files[0])
        },
        async deleteCat() {
            $('.loader').fadeIn().css('display', 'flex')
            try {
                const response = await axios.post(`{{ route("admin.restaurants.delete") }}`, {
                    id: this.id
                },
                );
                if (response.data.status === true) {
                    ...
                    setTimeout(() => {
                        $('.loader').fadeOut()
                        $('#errors').fadeOut('slow')
                        window.restaurant.href = '{{ route("admin.restaurants.show") }}'
                    }, 1300);
                } else {
                    $('.loader').fadeOut()
                    document.getElementById('errors').innerHTML = ''
