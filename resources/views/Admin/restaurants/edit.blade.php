...
<div class="card p-3 mb-3" id="restaurants_wrapper">
    ...
    <div class="form-group">
        <br>
        <button class="btn btn-success w-25" @click="updateRestaurant" style="margin: auto; display: block">Update</button>
    </div>
</div>
...
createApp({
    data() {
        return {
            ...
            working_to: "{{$restaurant->working_to}}",
            working_from: "{{$restaurant->working_from}}",
        }
    },
    methods: {
        ...
        async updateRestaurant() {
            ...
            try {
                const response = await axios.post(`{{ route("admin.restaurants.update") }}`, {
                    id: "{{$restaurant->id}}",
                    title: this.title,
